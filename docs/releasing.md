...
从 0.4.0 起已接入更新功能，当前为 0.6.0 预览版，尚未完成 Developer ID 签名、公证和两个真实签名版本之间的升级验证。旧 0.2.0 没有更新功能，需要手动安装一次包含更新功能的正式版。本机试用包可以生成 DMG，但不能作为稳定更新源。

预览版也有“软件更新”按钮，但只提供版本记录与下载入口，不能原地自动安装。启用现有自动升级链路需要配置Developer ID签名和Apple公证，通过两个正式签名版本间的实际升级验证，再手动安装一次正式版作为起点。不通过关闭签名检查或任意替换脚本来更新预览包。
