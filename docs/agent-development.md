...
代码审查为受限工具模式：保留宿主允许的读取/查询工具、固定CodeGraph查询、只读任务查询及飞书离线帮助，拒绝Bash、写入、检查与飞书业务调用。用户要求修复时先明确确认新建/恢复其他角色的实施任务，再遵守原方案批准；旧code-review任务不会被静默升级。其余开发角色也须经过共同的方案和验收门槛。
...
当前四个角色共用六个插件（含飞书 CLI）。Trellis 经自有 `trellis-product.mjs` 装配：保留上游运行能力，不注入第二套开发流程。产品只加载显式列出的工作法、接手、飞书及 Ponytail 技能；开发平台用的 `trellis-*` 技能仍保留在源码，不自动注入员工会话。角色提示词决定工作重点，readOnly标识同时控制真实活动工具及参数级门禁。公共getActiveTools/setActiveTools仅收窄会话初始工具集合，角色恢复不重启用宿主排除或随后手动禁用的工具。默认角色变更立即同步，但未完成任务的角色优先。此限制不是扩展进程/系统沙箱，也不是任意插件安装能力。
